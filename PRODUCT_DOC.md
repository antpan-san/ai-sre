...
普通用户登录后进入 `/app/dashboard`，只看到工作台信息架构：概览、执行记录、工作负载、能力中心、问题排查、作业中心、设置。管理后台 `/admin` 保留给 `admin` / `super_admin`，用户管理、订阅包配置、技能精炼和自动迭代管理不出现在普通用户侧栏。普通用户可在「能力中心」看到全部能力卡片、所属订阅包和订阅状态，未订阅时只展示说明、命令模板和订阅/联系管理员入口，执行仍由后端强校验。
