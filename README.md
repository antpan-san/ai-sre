...
**概览**：控制台不把「业务侧上报托管机器」做成全租户机器大盘；概览中的机器类 KPI 不再从 `Machine` 表聚合。**仅 `super_admin`** 在概览额外看到 **当前 OpsFleet 后端进程所在主机** 的 CPU、内存与根分区磁盘使用率（服务端即时采样，便于观察控制台机自身负载）；其它角色不展示该区块。业务服务台账在 **业务服务状态** 中按 **运行 / 部署中 / 停止 / 异常** 分色占比展示；**近 24h 执行** 的副文案会标出同口径下的 **失败** 条数（`GET /api/dashboard` 的 `platformSummary.executionsFailedLast24h`）。下方列表区各卡标题均为 **「最近 …」** 句式，表头与内边距与 KPI 区一致。
