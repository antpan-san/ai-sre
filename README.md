...
**Kubernetes 部署（推荐）**：在 **Kubernetes 部署** 向导中填写参数与节点 IP，点击 **「生成并下载离线安装包（zip）」**；在 **Ubuntu 24.04** 上解压后执行 **`sudo bash install.sh`** 完成 Ansible 编排安装（包内含 `ansible-agent` Playbook 与根据表单生成的 `inventory`）。**运行 `install.sh` 的机器须已能免密 SSH 登录各节点 `root`**（向导第 1 步有前置说明；`install.sh` 会在运行 Ansible 前预检 SSH）。**原仓库内 Go Agent（ft-client）源码已移除**；在线部署仍依赖 Agent 心跳上报时，可在同页切换到「在线 Agent」并提交在线部署。
