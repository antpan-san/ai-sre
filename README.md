...
- **ai-sre CLI**：`ai-sre node tune <subcmd>` 子命令在控制机或目标节点上 `sudo` 执行；当前已实现 **time-sync** 与 **sys-param**（>= 0.4.6），命令在 Go 内构建与 Ansible Tab 等价的 inventory + playbook，**先跑 `ansible-playbook --syntax-check`** 再正式执行；缺失则按 apt/dnf/yum 自动安装（可用 `--auto-install-ansible=false` 关闭、`--dry-run` 仅打印）。**security** 与 **disk** 仍是 roadmap，等子命令落地后 ai-sre 自动升级会随版本带过去
