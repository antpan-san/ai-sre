...
**前端路由与计费（可选）**：**管理员** 使用 **`/admin/...`**（全量运维菜单与 **「功能与计费」**），**普通用户** 使用 **`/app/...`**（仪表盘、作业中心、初始化工具、部署错误码等）；旧顶层路径会重定向。后端在 **`ft-backend/conf/config.yaml`** 的 **`billing`** 段配置 Stripe（`stripe_secret_key`、`stripe_webhook_secret`、`stripe_price_id_pro`、`public_app_base_url` 等）；管理端开关对应 **`GET/PUT /ft-api/api/admin/billing/features`**，当前用户状态 **`GET /ft-api/api/billing/me`**，Stripe Checkout **`POST /ft-api/api/billing/checkout-session`**，Webhook **`POST /ft-api/api/billing/stripe/webhook`**。默认 **`feature.advanced`** 计费为关，未开时行为与升级前一致。
