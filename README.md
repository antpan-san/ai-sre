...
**概览**：控制台不把「业务侧上报托管机器」做成全租户机器大盘；概览中的机器类 KPI 不再从 `Machine` 表聚合。**仅 `super_admin`** 在概览额外看到 **当前 OpsFleet 后端进程所在主机** 的 CPU、内存与根分区磁盘使用率（服务端即时采样，便于观察控制台机自身负载）；其它角色不展示该区块。业务服务台账在 **业务服务状态** 中与三列服务端资源同排展示（不占满整行）。**近 24h 执行** KPI 与 **近 24h 执行健康** 条展示新建条数、成功/已取消/失败及终态失败率，并按 **source** 汇总 `cli` / `k8s` / `job` 条数（与列表同一角色可见范围；`GET /api/dashboard` 的 `platformSummary.executionsSuccessLast24h`、`executionsCancelledLast24h`、`executionsFailedLast24h`、`executionsBySourceLast24h`）。下方列表区各卡标题均为 **「最近 …」** 句式；**最近 Linux / 业务服务** 与 **最近执行记录** 两表各占一整行以利宽表展示；表头内边距与 KPI 区一致。
