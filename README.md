...
Go CLI + 同仓 **OpsFleetPilot** Web/API（`ft-backend/`、`ft-front/`、`deploy/`、`ansible-agent/`）。核心能力：**AI 故障诊断**（`check`）、**只读快采**（`probe`）、**Runbook**（`runbook`）、**问答**（`ask`）；**技能**（`skills`）；**K8s 离线安装**（`k8s`）；**批量作业**（`job run`）。旧命令 `analyze` / `topic diagnose` / 顶层 `diagnose` 仍可用但已弃用。
...
| `ai-sre skills feedback --topic <t> -m "…"` | 提交诊断反馈样本；可经 `POST /api/cli/feedback/analyze` 触发自动迭代研判 |
...
./ai-sre check kafka -d bootstrap=broker1:9092 -d topic=orders
./ai-sre check redis -d target=127.0.0.1:6379
./ai-sre check mysql -d dsn='user:pass@tcp(127.0.0.1:3306)/db?charset=utf8mb4'
./ai-sre check elasticsearch -d base_url=http://127.0.0.1:9200
