...
控制台：登录后在侧栏打开 **「部署错误码」**（工作台 **`/app/help/error-codes`**，旧路径 **`/help/error-codes`** 会重定向）可浏览与搜索同源目录；接口仍为 `GET /ft-api/api/ai/error-codes`，单条根因卡 `POST /ft-api/api/ai/error-codes/analyze {code,detail}`；客户端等价命令是 `ai-sre analyze code <CODE> [--detail "…"]`，输出三段式：根因 / 立即恢复一行 / 平台改进+文件路径（不给排查清单）。**前端页头与纵向留白约定**见仓库 **`.cursor/rules/ft-front-compact-layout.mdc`**（`ft-front` 新页与改版须遵守）。
...
**前端路由**：全体登录租户成员（含 `user`）默认进入 **`/admin/...`** 并使用与管理员同一套侧边栏与各功能页（与接口能力一致时才可操作）；**`/app/...`** 工作台仍可作兼容书签。路由级禁区：**用户列表**、**权限管理** 仅限 **`admin` / `super_admin`**；**功能与计费** 仅限 **`super_admin`**。**`super_admin`** 永久豁免功能计费，并独占 **「功能与计费」** 与手动授予权益。功能键分为 **`feature.k8s_ops`**（K8s 交付相关 API）、**`feature.service_ops`**（服务部署与 Linux 服务）、**`feature.infra_ops`**（代理、监控、初始化工具）、**`feature.advanced`**（备份与性能）；各键在 **`GET/PUT /ft-api/api/admin/billing/features`** 中独立开关是否启用计费。Stripe 支持 **多功能包**：在 **`ft-backend/conf/config.yaml`** 的 **`billing.packages`** 为每个 Stripe Price 配置 `id`、`display_name`、`stripe_price_id` 与 `feature_keys` 列表；若 `packages` 为空且仍配置 **`stripe_price_id_pro`**，则向后兼容为单包仅含 **`feature.advanced`**。对外接口：`GET /ft-api/api/billing/packages`（档位列表）、`GET /ft-api/api/billing/me`（含 `feature_access`）、`POST /ft-api/api/billing/checkout-session`（Body 可选 `package_id`）、`POST /ft-api/api/billing/stripe/webhook`。高危写操作在后端另以 **`RequireAdmin()`** 约束（如注册用户、执行任务、下发部署与 K8s、机器增删改、执行记录回滚、RBAC 写入等）。默认各键计费为关。详细设计见 **`docs/advanced-feature-billing-design.md`**。
