...
**部署前的节点初始化（可选但推荐）**：在 **节点配置 → 下一步** 时控制台会提示是否先去优化节点环境（避免 etcd / calico-node / coredns 在 NTP 漂移、br_netfilter 缺失、慢盘等情况下反复 Killing）。点击「先去优化」会跳转到 **初始化工具 → 工具总览**（`/init-tools`），其中每张卡片为一个优化项（**时间同步 / 系统参数优化 / 系统安全加固 / 磁盘分区优化**），可分别选择**目标节点**与**系统类型**（Ubuntu/Debian/CentOS/Rocky/RHEL/openEuler/Kylin/其它 Linux），完成后点顶部「返回 K8s 部署」继续向导。对话框支持「以后不再提示」（写入 localStorage）。
