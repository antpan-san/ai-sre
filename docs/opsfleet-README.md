...
| 变量 | 默认 | 说明 |
|------|------|------|
| `OPSFLEET_REMOTE` | `root@192.168.56.11` | SSH 目标 |
...
**当前默认部署节点**为 **`192.168.56.11`** 时，在虚拟机内使用 **`scripts/vmware-fusion-192.168.56-netplan.yaml`**（**`192.168.56.11/24`**，网关 **`192.168.56.2`**），再 `sudo netplan apply`。若 Fusion 仍为默认 **NAT（vmnet8）** `172.16.195.0/24`，需在 **Mac 上**用 `sudo` 执行 **`scripts/vmware-fusion-nat-to-192.168.56.sh`**（先关虚拟机与 Fusion），把 vmnet8 改为 **`192.168.56.0/24`**，否则 **Mac 与虚拟机无法互通**。

- **备选（Fusion 仍为默认 vmnet8 `172.16.195.0/24` 且不改子网）**：使用 **`scripts/vmware-fusion-vmnet8-netplan.yaml`**（如 **`172.16.195.11/24`**，网关 **`172.16.195.2`**），再 `sudo netplan apply`；此时请将 `DEPLOY_REMOTE` / `OPSFLEET_REMOTE` 指向该虚拟机实际 IP，而非 `192.168.56.11`。
