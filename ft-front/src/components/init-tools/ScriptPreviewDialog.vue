<template>
  <!--
    ...
    - Tab 3: ai-sre CLI — 是否可执行由 bundle.aiSreCommandExecutable 决定：
        time-sync / sys-param 已在 ai-sre 中实现（>=0.4.8），可直接 sudo bash 执行；
      ...
      <el-alert type="info" :closable="false" class="dialog-hint">
        <template #title>
          <span>
            ...
            <strong>「ai-sre CLI」</strong>当此卡片在 ai-sre 中已实现时（time-sync / sys-param ≥ 0.4.8）也可 <code>sudo bash</code> 直接运行，
...
const cliReadyHint =
  'ai-sre 0.4.8 起已内置 time-sync / sys-param 并修正 playbook YAML 解析与「skip 误跳时区」问题；time-sync 只对 --clients 列表的节点动手，不会自动包括跑 ai-sre 的本机；执行前还会跑 ansible-playbook --syntax-check。低版本节点上首次执行命令会自动升级到最新版（OPSFLEET_NO_AUTO_UPGRADE 未设置时）。' +
