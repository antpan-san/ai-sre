    ...
    >
      ...
      >
        ...
        <el-form-item label="JVM 参数">
          <el-input
            v-model="formData.jvmArgs"
            type="textarea"
            placeholder="请输入JVM参数"
            :rows="3"
          />
        </el-form-item>
      </el-form>
...
import type { JmxExporterConfig } from '../../types/monitoring'
...
const formData = reactive({
  ...
  enabled: true,
  port: 9404,
  configFile: '/etc/jmx-exporter/config.yml',
  jvmArgs: ''
})
...
const formTitle = computed(() => {
  return currentConfigId.value ? '编辑 JMX Exporter 配置' : '新增 JMX Exporter 配置'
})

// 筛选 JMX Exporter 配置（全部）
const allJmxExporterConfigs = computed(() => {
  return monitoringStore.configs.filter(config => config.type === 'jmx-exporter') as JmxExporterConfig[]
})

// 分页后的配置
const jmxExporterConfigs = computed(() => {
  const start = (currentPage.value - 1) * pageSize.value
  const end = start + pageSize.value
  return allJmxExporterConfigs.value.slice(start, end)
})
...
const fetchConfigs = async () => {
  ...
  try {
    ...
    total.value = allJmxExporterConfigs.value.length
  } catch (error) {
...
// 处理状态变更
const handleStatusChange = async (row: JmxExporterConfig) => {
  loading.value = true
  try {
...
// 处理编辑配置
const handleEditConfig = (row: JmxExporterConfig) => {
...
// 处理删除配置
const handleDeleteConfig = (row: JmxExporterConfig) => {
